...
object BadRow {
  ...
  implicit val bqLoaderBadRowCirceJsonEncoder: Encoder[BadRow] =
    Encoder.instance {
      ...
      case RuntimeError(original, errorInfos) => json"""{
          "event": $original,
          "error": ${errorInfos.asJson.noSpaces},
          "processor": $BQLoaderProcessorInfo
        }"""
    }
  ...
  /**
    ...
    * @param event event which is worked on when error is got
    * @param errorInfos list of error info objects which contains reasons of errors
    */
  final case class IgluError(event: Event, errorInfos: NonEmptyList[IgluErrorInfo]) extends BadRow {
    def getSelfDescribingData: SelfDescribingData[Json] =
  ...
  /**
    ...
    * @param schema key of schema which is worked with when error got
    * @param error error object from Iglu client which gives information about error
    */
  final case class IgluErrorInfo(schema: SchemaKey, error: ClientError)
